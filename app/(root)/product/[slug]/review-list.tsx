...
const ReviewList = ({userId, productId, productSlug} : {
    ...
    //Recargar la review enseguida que se actualiza o se crea.
    const reload = async ()=>{
        const res = await getReviews({productId});
        setReviews([...res.data]);
    }
    ...
    return ( <div className="space-y-4">
        {reviews.length === 0 && <div> No hay reviews todavia</div>}
        {userId ? (
            <>
        ...
        <div className="flex flex-col gap-3">
            {reviews.map((review)=>(
                <Card key={review.id}>
                    ...
                    <CardContent>
                        <div className="flex space-y-4 text-sm text-muted-foreground">
